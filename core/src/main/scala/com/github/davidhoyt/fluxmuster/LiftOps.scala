...
trait LiftOps[State, Into[_]] {
  ...
  def runInThisContext[A, D, From[_]](linkChain: LinkChain, opsChain: ChainedLiftOps[Into], otherRunner: A => From[D], state: State)(implicit converter: From -> Into, typeIn: TypeTagTree[A], typeFromOut: TypeTagTree[From[D]], typeIntoOut: TypeTagTree[Into[D]]): Link[A, Into[D]] = {
    Link((in: A) => {
      val runOtherInThisContext: A => Into[From[D]] = liftRunner(linkChain, opsChain, otherRunner)(state, typeIn, typeFromOut)
      val resultAfterRunning: Into[From[D]] = runOtherInThisContext(in)

      logger.debug(s"LiftOps.runInThisContext result after running: $resultAfterRunning")

      val mapResultBackIntoThisContext: Into[Into[D]] = map(resultAfterRunning) { from: From[D] =>
        logger.debug(s"LiftOps.runInThisContext converting $from [${typeFromOut.toShortString}]")
        val converted: Into[D] = converter.apply(from)
        logger.debug(s"LiftOps.runInThisContext converted to $converted [${typeIntoOut.toShortString}]")
        //could it be an issue of the map and flatten running async in parallel?
        //no
        converted
      }(state)

      val flattenedBackIntoThisContext: Into[D] =
        flatten(mapResultBackIntoThisContext)(state)
  ...
  implicit def asLiftOpsAny: LiftOpsAny =
    this.asInstanceOf[LiftOpsAny]
}